...
def do_ssh(bot=None, chat_id=None, remote=None, ssh_user=None, ssh_password=None):
    bot.send_message(chat_id, "\xF0\x9F\x98\x8C Bien, verificare si <b>"+ remote +"</b> esta accesible"
                     , parse_mode="Html")
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    if has_ping(remote):
        bot.send_message(chat_id, "\xE2\x9C\x8C \xF0\x9F\x8E\x89 \xF0\x9F\x8E\x8A 	\xF0\x9F\x91\x8F \n"
        ...
        while 1 and i < 3:
            try:
                ...
                bot.send_message(chat_id, "\xF0\x9F\x98\xA8 \xF0\x9F\x91\xAE Fallo la autenticacion al conectarse a: %s" % remote)
                break
            except Exception, e:
                ...
                bot.send_message(chat_id, face_str +"<i> No pude establecer conexion con el destino \n"
                                          "\xF0\x9F\x98\x87 Intentare una vez mas </i>", parse_mode="Html")
                i += 1
                time.sleep(2)
            if i >=3:
                bot.send_message(chat_id, "	\xF0\x9F\x98\x94 \xF0\x9F\x92\x94 <i>Hijole!, hice lo posible, pero no me fue posible "
                                          "conectar con: %s, no hubo quimica entre nosotros </i> \xF0\x9F\x98\x85" % remote, parse_mode="Html" )
        return  ssh, conn


    else:
        bot.send_message(chat_id, "\xF0\x9F\x98\xB2 \xE2\x9D\x8C \xF0\x9F\x92\x80"
...
def get_sys_info(bot=None, chat_id=None, remote=None, ssh_user=None, ssh_password=None):
    ...
    if conn:
        ...
        bot.send_message(chat_id, "	\xF0\x9F\x98\x94 \xF0\x9F\x92\x94 <i>Hijole!, no logre conectar con %s"
...
def get_load_avg(ssh=None, bot=None, chat_id=None):
    try:
        ...
        bot.send_message(chat_id, "\xF0\x9F\x98\x8E <b>Carga primedio: %s </b>"
...
def get_cpu_usage(ssh=None, bot=None, chat_id=None):
    try:
        ...
        bot.send_message(chat_id, "\xF0\x9F\x98\x8E <b>Uso de CPU: %s</b> " % usage_str, parse_mode="Html")
    except Exception, e:
        print(e.message)
        bot.send_message(chat_id, "\xF0\x9F\x92\xA5 \xF0\x9F\x98\xB5 <b>Ocurrio un inconveniente "
...
def get_disk_usage(ssh=None, bot=None, chat_id=None):
    try:
        ...
        bot.send_message(chat_id, "\xF0\x9F\x98\x8E <b>Uso de disco particion /: %s</b> " % percent, parse_mode="Html")
    except Exception, e:
        print(e.message)
        bot.send_message(chat_id, "\xF0\x9F\x92\xA5 \xF0\x9F\x98\xB5 <b>Ocurrio un inconveniente "
